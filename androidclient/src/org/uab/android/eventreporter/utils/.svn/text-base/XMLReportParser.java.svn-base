...
public class XMLReportParser extends DefaultHandler {
	...
	public static String encodeConfirmation(String position, String username) {
		String[] parts = position.split(",");
		StringBuilder encodedXML = new StringBuilder();
		
		encodedXML.append(PREAMBLE).append("\n");
		
		encodedXML.append(OPEN_TAG).append(CONFIRMATION_TAG).append(CLOSE_TAG);
		
		encodedXML.append(OPEN_TAG).append(GPS_TAG).append(" ");
		encodedXML.append(GPS_LATITUDE).append(parts[0]).append("\"").append(" ");
		encodedXML.append(GPS_LONGITUDE).append(parts[1]).append("\"").append(CLOSE_TAG);
		encodedXML.append(END_TAG).append(GPS_TAG).append(CLOSE_TAG);
		
		encodeTag(encodedXML, USERNAME_TAG, username);
		
		encodedXML.append(END_TAG).append(CONFIRMATION_TAG).append(CLOSE_TAG);
